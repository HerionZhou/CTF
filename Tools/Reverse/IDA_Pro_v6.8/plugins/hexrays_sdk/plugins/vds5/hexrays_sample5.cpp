    ...
    case hxe_open_pseudocode:
      {
        ...
        attach_action_to_popup((TForm*) vu.ct, NULL, ACTION_NAME);
      }
      break;
...
struct display_graph_ah_t : public action_handler_t
{
  ...
  {
    vdui_t *vu = get_tform_vdui(ctx->form);
    return vu == NULL ? AST_DISABLE_FOR_FORM : AST_ENABLE_FOR_FORM;
  }
};
static display_graph_ah_t display_graph_ah;
static const action_desc_t display_graph_action = ACTION_DESC_LITERAL(
        ACTION_NAME, "Display graph", &display_graph_ah, ACTION_SHORTCUT,
        NULL, -1);
